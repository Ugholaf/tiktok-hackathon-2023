                    ...
                    {transactions?.transactionsIn?.slice(0,).map((transaction) => (
                        <div key={transaction.id} className="flex text-ellipsis justify-between gap-2 items-center self-stretch border-b border-neutral-300 mb-2">
                            <div className="flex flex-col min-w-[90px] lg:flex-row justify-between items-start lg:min-w-[160px]">
                                <p className="text-sm max-w-[90px] lg:text-base lg:max-w-[200px] ">{(new Date(transaction.createdAt).toLocaleDateString('en-GB').split("T")[0])} &nbsp; </p>
                                <p className="text-sm max-w-[90px] lg:text-base lg:max-w-[200px] ">{(new Date(transaction.createdAt).toISOString().split("T")[1].slice(0, -5))}</p>
                            </div>
                            <div className="flex justify-center items-center">
                                <p className="text-sm lg:text-base">{transaction.type.toLowerCase().replace(/_/g, " ")}</p>
                            </div>
                            <div className="flex justify-end max-w-[62px] lg:min-w-[160px]">
                                <p className="text-sm lg:text-base text-right">{transaction.amount.toFixed(2)} {transaction.currency} </p>
                            </div>
                        </div>
